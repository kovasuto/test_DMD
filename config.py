...
class Config:
    ...
    # 参照メッシュ(座標+セル接続情報)のファイル名。モード形状・時系列再構築の
    # VTK出力で、このメッシュに値を載せることで、点群ではなくセル情報付きの
    # (ParaViewで面として表示できる)VTKを出力できるようになる。
    mesh_topology_name: str = "mesh_topology.vtu"
    ...
    # 8. 再構築結果のVTK時系列出力設定
    # ------------------------------------------------------------------
    # True: 時系列再構築結果を、参照メッシュ(セル接続情報付き)+ .pvd時系列
    #       コレクションとしてVTK出力する。ParaViewで.pvdファイルを開くと、
    #       アニメーションとして時間発展を再生できる。
    reconstruction_export_vtk: bool = True

    # 再構築したタイムステップのうち、何ステップおきにVTKへ書き出すか。
    # 1なら全ステップ、2なら1つおき、というように間引く
    # (再構築のタイムステップ数が多い場合、VTKファイル数・ディスク容量を抑える)
    vtk_export_stride: int = 1

    # VTK出力するフレーム数の上限(None なら上限なし)。
    # 大量のタイムステップを再構築した場合に、ディスク容量・出力時間を
    # 抑えるための安全弁。上限を超える場合は時間方向に均等間引きして書き出す。
    vtk_export_max_frames: Optional[int] = 100

    reconstruction_vtk_dir_name: str = "reconstruction_vtk"

    # ------------------------------------------------------------------
    # 9. 無次元化(Strouhal数)設定
    # ------------------------------------------------------------------
    # St = f * L / U  (f: 周波数[Hz], L: 代表長さ[m], U: 代表流速[m/s])
    # DMDモードの周波数一覧・VTKファイル名・診断プロットに、Hzに加えて
    # St数も併記できるようにするための設定。対象形状の代表長さ(車高・
    # 直径など論文/社内基準に合わせたもの)と自由流速を指定すること。
    reference_length: float = 1.0   # 代表長さ L [m]
    reference_velocity: float = 1.0  # 代表流速 U [m/s]
