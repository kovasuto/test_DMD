...
def select_modes(result: dict, cfg: Config) -> dict:
    ...
    st_selected = freq_to_strouhal(result["freq_hz"], cfg)

    print(f"\n[モード選択] 方法 = {method}, 選択モード数 = {idx_selected.size}")
    print(
        f"{'rank':>4} {'idx':>5} {'freq[Hz]':>10} {'St':>10} "
        f"{'growth_rate':>12} {'|b|':>12}"
    )
    for rank, i in enumerate(idx_selected):
        print(
            f"{rank:>4} {i:>5} {result['freq_hz'][i]:>10.4f} {st_selected[i]:>10.4f} "
